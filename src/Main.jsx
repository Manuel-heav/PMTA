...
const Main = () => {
  ...
  const findMovie = async (e) => {
    ...
    setImageUrl(movie?.results[num]?.poster_path);
    setMovieTitle(movie?.results[num]?.title)
    setNum(num + 1);
  ...
  //     setUrl(await movieTrailer(movieTitle))
  // }
  // getTrailer();
  }
  console.log(movieTitle)

  const properUrl = trailerUrl?.slice(32);
  ...
  const addMovie = (e) => {
    ...
    db.collection('items').add({
      desc: desc,
      title: movieTitle ,
      trailerUrl: properUrl,
      username: username,
      genre: genre,
    })
    alert("Movie added successfully")
  }
  const imgUrl = base_img_url + imageUrl;
  console.log(imgUrl)
  ...
  <div className="space-y-4">
    ...
    <div className="flex space-x-4">
      <div className="flex-1">
        <label className="block text-sm font-medium mb-1" htmlFor="card-expiry">Enter Trailer Link<span className="text-red-500">*</span></label>
        <input value={trailerUrl} onChange={e => setTrailerUrl(e.target.value)} id="card-expiry" className="text-sm text-gray-800 bg-white border rounded leading-5 py-2 px-3 border-gray-200 hover:border-gray-300 focus:border-indigo-300 shadow-sm placeholder-gray-400 focus:ring-0 w-full" type="text" placeholder="What is the title of the movie?" />
      </div>
    </div>
  ...
  <div className="mt-6">
   ...
   <p>{desc}</p>
   {
    imgUrl === ("https://image.tmdb.org/t/p/w500" || "https://image.tmdb.org/t/p/w500undefined" || "undefined" || "") ? <div><p>Nothing to fetch</p></div>  : <img src={imgUrl}/>
   }
  </div>
</div>
    </div>
